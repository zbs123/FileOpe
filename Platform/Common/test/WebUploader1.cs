...
namespace Platform.Commonn.test
{
    #region 辅助类
    /// <summary>
    /// 上传消息类
    ...
    public class WebUploaderField
    {
        ...
        public string FileSaveChunkPath
        {
            get
            {
                ...
                {
                    strpath = AppDomain.CurrentDomain.BaseDirectory + "upload\\" + Filename + "_" + Md5 + "_";
                }
                else
                {
                    strpath = AppDomain.CurrentDomain.BaseDirectory + "upload\\"+Filename+"_";
                }
                return strpath;
            }
        }
    ...
    public class WebUploader
    {
        ...
        /// <returns></returns>
        public UploaderResult Process(HttpRequestBase request, string saveFileName, Func<string, bool> dataIn, string savepath = null
            , Func<HttpPostedFileBase, string> setfilename = null, Func<string, bool> md5check = null)
        {
            InitWebUploaderField(request, savepath, saveFileName);
            ...
            if (_webUploaderField.Chunks != 0)
            {
                ...
                if (WebUploader.allChunks[_webUploaderField.FileSavePath].Count == _webUploaderField.Chunks)
                {
                    ...
                    using (FileStream fsw = new FileStream(NewFileName(file), FileMode.Create, FileAccess.Write, FileShare.Write))
                    {
                    ...
                    dataIn(_webUploaderField.FileSavePath);
                    obj.Filish = true;
        ...
        public string NewFileName(string filepath)
        {
            FileInfo fi = new FileInfo(filepath);
            string tempP = filepath;
            int count = 1;
            while (File.Exists(tempP))
            {
                tempP = filepath;
                tempP =tempP.Substring(0, tempP.LastIndexOf('.'))+ "(" + count + ")"+fi.Extension;
                count++;
            }
            return tempP;
        }
